...
- 默认文档根目录：`C:\Users\Obsidian\Codex`。
- 默认按“当前仓库所在文件夹层级”在该根目录下建立子目录后再落文档；例如仓库 `D:\Documents\Code/stock\trading_system` 的默认文档目录为 `C:\Users\Obsidian\Codex\stock\trading_system`。
- 若当前仓库路径中包含 `Code\`，默认取其后的相对层级作为子目录；若不包含，则回退为“仓库父目录名\仓库目录名”。若用户明确指定其他路径，以用户要求为准。
